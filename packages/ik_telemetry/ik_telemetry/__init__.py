...
def setup_telemetry(settings_or_service_name="indus-kernel", endpoint: str | None = None) -> bool:
    ...
    if endpoint is None:
        endpoint = getattr(settings_or_service_name, "otel_exporter_otlp_endpoint", None)
    try:
        ...
        logging.getLogger(__name__).warning(
            "OpenTelemetry unavailable; using stdlib telemetry: %s", exc
        )
        _configured = True
        return False
...
def get_tracer(name: str = "indus"):
    try:
        ...
        class _Noop:
            @contextmanager
            def start_as_current_span(self, *args, **kwargs) -> Iterator[None]:
                yield None
