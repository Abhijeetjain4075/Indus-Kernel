...
class AgentRunStore:
    ...
    async def _db(self):
        ...
        self._pool = await asyncpg.create_pool(
            url, min_size=1, max_size=max(2, s.database_pool_size), command_timeout=5
        )
        return self._pool
    ...
    async def get_by_idempotency(self, tenant_id: str, key: str | None) -> Any | None:
        ...
        if s.environment in {"staging", "production"}:
            ...
            return await pool.fetchrow(
                "SELECT * FROM agent_runs WHERE tenant_id=$1 AND idempotency_key=$2", tenant_id, key
            )
    ...
    async def list(self, tenant_id: str, limit: int, offset: int, admin: bool = False) -> list[Any]:
        ...
        if s.environment in {"staging", "production"}:
            ...
            if admin:
                ...
                rows = await pool.fetch(
                    "SELECT * FROM agent_runs WHERE tenant_id=$1 ORDER BY started_at DESC LIMIT $2 OFFSET $3",
                    tenant_id,
                    limit,
                    offset,
                )
            return list(rows)
        runs = (
            list(self._memory.values())
            if admin
            else [r for r in self._memory.values() if r.tenant_id == tenant_id]
        )
