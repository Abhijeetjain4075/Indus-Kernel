...
class RateLimiter:
    def __init__(self, redis_url:str, limit:int)->None:
        ...
        if Redis is not None:
            try: self.redis=Redis.from_url(redis_url,decode_responses=True)
    ...
    async def allow(self,key:str)->tuple[bool,int]:
        ...
        if self.redis is not None:
            ...
            if count==1: await self.redis.expire(redis_key,70)
        ...
        if len(self._local)>10000:
            for k in list(self._local)[:5000]: del self._local[k]
        return count<=self.limit,max(0,self.limit-count)
    async def close(self)->None:
        if self.redis is not None: await self.redis.aclose()
