...
def _row_to_run(row) -> AgentRunResponse:
    return AgentRunResponse(
        run_id=row["id"], tenant_id=row["tenant_id"], status=row["status"], goal=row["goal"], topology=row["topology"],
        result=row["result"], error=row["error"], total_tokens=row["total_tokens"], total_cost_cents=row["total_cost_cents"],
        total_latency_ms=row["total_latency_ms"], started_at=row["started_at"], completed_at=row["completed_at"], idempotency_key=row["idempotency_key"],
    )
...
async def start_agent_run(req: AgentRunRequest, principal: Principal = Depends(get_current_principal)) -> AgentRunResponse:
    ...
    if existing is not None:
        ...
        return _row_to_run(existing) if get_settings().environment in {"staging", "production"} else existing
    run_id = _new_run_id()
    ...
    try:
        if req.topology == "hello":
            result = await run_hello_agent(goal=req.goal, run_id=run_id, user_id=principal.user_id or principal.tenant_id, session_id=run_id)
            ...
            answer = f"Topology={req.topology}.\nPlan:\n" + "\n".join(f"- {s.id}: {s.title}" for s in plan.steps) + f"\nReasoning:\n{rr.conclusion}"
            total_tokens = 0
            total_cost = 0
            latency = 0
...
async def get_agent_run(run_id: str, principal: Principal = Depends(get_current_principal)) -> AgentRunResponse:
    ...
    if get_settings().environment in {"staging", "production"}:
        ...
        result = run
    if "admin" not in principal.roles and "*" not in principal.scopes and result.tenant_id != principal.tenant_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="run not found")
    return result


@router.get("/runs", response_model=list[AgentRunResponse], summary="List agent runs")
async def list_agent_runs(limit: int = 50, offset: int = 0, principal: Principal = Depends(get_current_principal)) -> list[AgentRunResponse]:
    """List recent agent runs (tenant scoped)."""
    from ik_kernel.config import get_settings
    limit = min(max(limit, 1), 100)
    rows = await get_run_store().list(principal.tenant_id, limit, max(offset, 0), admin=("admin" in principal.roles or "*" in principal.scopes))
    if get_settings().environment in {"staging", "production"}:
...
async def cancel_agent_run(run_id: str, principal: Principal = Depends(get_current_principal)) -> AgentRunResponse:
    from ik_kernel.config import get_settings
    run = await get_run_store().get(run_id)
    if run is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="run not found")
...
async def stream_agent_run_events(run_id: str, principal: Principal = Depends(get_current_principal)):
    ...
    if "admin" not in principal.roles and "*" not in principal.scopes and run.tenant_id != principal.tenant_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="run not found")
