...
from .ttcs import _default_answer_extractor
...
class EvalReport:
    ...
    def summary(self) -> str:
        lines = [
            f"Model: {self.model_version} ({self.parameter_count / 1e6:.2f}M params)",
...
def eval_perplexity(
    ...
    return BenchmarkResult(
        "perplexity", ppl, 1, ppl, 0.0, {"tokens": total_tokens, "nll": total_nll}
    )
...
def eval_code_completion(
    ...
    for t in applicable:
        ...
        out = model.generate(idx, max_new_tokens=max_new_tokens, temperature=temperature, top_k=10)
...
def eval_math(
    ...
    for t in applicable:
        ...
        idx = torch.tensor([tokenizer.encode(prompt)], dtype=torch.long, device=device)
        out = model.generate(
            idx, max_new_tokens=max_new_tokens, temperature=max(temperature, 0.1), top_k=20
        )
        new_ids = out[0].tolist()[idx.size(1) :]
        gen = tokenizer.decode(new_ids).strip()
        ...
        n_correct += int(hit)
        per_item.append(
            {"q": t["q"], "expected": t["a"], "got": extracted, "raw": gen[:60], "hit": hit}
        )
    score = n_correct / max(1, len(applicable))
    return BenchmarkResult(
        "math_exact_match",
