...
INDUS_CONFIGS = {
    ...
    "indus-tiny": IndusConfig(
        name="indus-tiny",
        n_layer=6, n_embd=156, n_head=6, n_kv_head=2,
        block_size=128, dropout=0.1, vocab_size=256,
        ffn_kind="swiglu",
    ),
    # ~50M params. Needs 1 GPU.
    "indus-small": IndusConfig(
        name="indus-small",
        n_layer=10, n_embd=768, n_head=12, n_kv_head=4,
        block_size=2048, dropout=0.1, vocab_size=50257,
        ffn_kind="swiglu",
    ),
    # ~125M params (GPT-2 small territory). Needs 1 A100.
    "indus-base": IndusConfig(
        name="indus-base",
        n_layer=12, n_embd=768, n_head=12, n_kv_head=12,
        block_size=2048, dropout=0.1, vocab_size=50257,
        ffn_kind="swiglu",
    ),
    ...
    "indus-moe-1b": IndusConfig(
        name="indus-moe-1b",
        n_layer=24, n_embd=2048, n_head=32, n_kv_head=8,
        block_size=32768, dropout=0.0, vocab_size=50257,
        ffn_kind="moe", moe_num_experts=8, moe_top_k=2,
    ),
    # ~7B params with 8-expert MoE. Needs 16x H100.
    "indus-moe-7b": IndusConfig(
        name="indus-moe-7b",
        n_layer=32, n_embd=4096, n_head=32, n_kv_head=8,
        block_size=131072, dropout=0.0, vocab_size=100352,
        ffn_kind="moe", moe_num_experts=8, moe_top_k=2,
