...
class SandboxPolicy:
    ...
    def validate(self):
        ...
        if self.memory_mb < 64:
            raise ValueError("memory_mb too low")
        return self
    ...
    async def execute(self, command: list[str], policy: SandboxPolicy | None = None):
        ...
        if not self.backend:
            raise SandboxUnavailable(
                "No isolated executor configured; production must provide Firecracker/gVisor/E2B backend"
            )
...
def execute(command: list[str], policy: SandboxPolicy | None = None):
    raise SandboxUnavailable(
        "Direct local execution of untrusted commands is prohibited; configure an isolated backend"
    )
